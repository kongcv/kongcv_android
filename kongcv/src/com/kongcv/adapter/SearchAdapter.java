...
public class SearchAdapter extends BaseAdapter {
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if (convertView == null) {
			...
			holder.ivDaoH = (ImageView) convertView.findViewById(R.id.iv_daoh);
			holder.txtFind = (TextView) convertView.findViewById(R.id.txtfind);
			holder.txtCity = (TextView) convertView
					.findViewById(R.id.search_tv_city);
			holder.ivCome = (ImageView) convertView.findViewById(R.id.iv_come);
			convertView.setTag(holder);// 绑定ViewHolder对象
		} else {
			holder = (ViewHolder) convertView.getTag();
		}
		// 给控件设置数据
		holder.ivDaoH.setImageResource((Integer) dataList.get(position).get(
				SearchActivity.KEY[0]));
		holder.txtFind.setText((CharSequence) dataList.get(position).get(
				SearchActivity.KEY[1]));
		holder.ivCome.setImageResource((Integer) dataList.get(position).get(
				SearchActivity.KEY[2]));
