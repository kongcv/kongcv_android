 ...
 */
public class CommunityDetailFragment extends Fragment implements OnClickListener,
	...
	private Handler mHandler = new Handler() {
		...
		public void handleMessage(Message msg) {
			...
			case 0:
				...
				if(GTMDateUtil.GTMToLocal(result.getHire_start()
						.getIso(), false)!=null && GTMDateUtil.GTMToLocal(result.getHire_end()
						.getIso(), false)!=null){
					txt_start.setText(GTMDateUtil.GTMToLocal(result.getHire_start()
							.getIso(), false));
				...
				if(result.getProperty()!=null){
					payOneBean.setProperty_id(result.getProperty().getObjectId());
				}
			//	Data.putData("PayOneBean", payOneBean);
				if(mineSendFragment!=null){
					...
					mInform.setVisibility(View.GONE);// 求租人 订单确认不可见
					mPay.setVisibility(View.VISIBLE);// 求租人 支付按钮可见
					JpushBean fromJson = gson.fromJson(mineSendFragment, JpushBean.class);
...
//					visiblePhone(stringExtra);
				...
				if(CurbMineReceiver!=null){
					JpushBean fromJson = gson.fromJson(CurbMineReceiver, JpushBean.class);
					String hire_start = fromJson.getHire_start();
					String hire_end = fromJson.getHire_end();
					time_pay_start.setText(hire_start);
					time_pay_end.setText(hire_end);
	...
	public void onClick(View v) {
		...
		case R.id.inform:// 点击发送订单
			if(mineSendFragment==null || strExtra==null || CurbMineReceiver==null){
		...
		case R.id.time_pay_start:
			dialog = new PickDialog(getActivity(), new PickDialogListener() {
				...
				public void refreshPriorityUI(String str) {
					...
					if (!start.equals(" 年  月  日") && !end.equals(" 年  月  日") && index!=-1) {
						...
						String text = (String) dataList.get(index).get(CommunityDetailFragment.KEY[2]);
						String a[] = text.split("/");
	...
	private void consentJPush() {
		...
		try {
			...
			okhttp3.Request request=new okhttp3.Request.Builder()
		      ...
		      .post(RequestBody.create(Information.MEDIA_TYPE_MARKDOWN, JsonStrUtils.JsonStr(obj)))
		      .build();
			client.newCall(request).enqueue(new okhttp3.Callback() {
				...
				public void onResponse(Call arg0, okhttp3.Response response) throws IOException {
					...
					if(response.isSuccessful()){
						try {
							JSONObject obj2 = new JSONObject(response.body().string());
							String str = obj2.getString("result");
							JSONObject objStr = new JSONObject(str);
							String state = objStr.getString("state");
							if ("ok".equals(state)) {
	...
	private void Details() {
		/*try {
			...
			if (doHttpsPost.equals("{}")) {
				...
				Gson gson = new Gson();
				DetailBean bean = gson.fromJson(doHttpsPost, DetailBean.class);
	...
	@Override
	public void onItemClick(AdapterView<?> parent, View parentView, int position,
			long id) {
		start = time_pay_start.getText().toString();
		end = time_pay_end.getText().toString();
		if (start.equals(" 年  月  日") || end.equals(" 年  月  日")) {
	 ...
	 *计算时间差
	 */
	private String getDays(String start,String end,String[] notRestArr,String days){
		int day=0;
		int parseInt=0;
		try {
	        ...
	        List<java.util.Date> lists = DateUtils.dateSplit(startDate, endDate);
	        if(notRestArr!=null){
	        	for(int i=0;i<notRestArr.length;i++){
		        	 if (!lists.isEmpty()) {
		        		 for (Date d : lists) {	
		        			 if(notRestArr[i].equals(DateUtils.getWeekday(sdf.format(d)))){
			    					day++;
			    			 }
		        		 }
		        	 }
		...
		new Thread(new Runnable() {
			...
			public void run() {
				...
				// TODO Auto-generated method stub
					...
					object.put("park_id", jpushBean.getPark_id());
					object.put("mode", jpushBean.getMode());
					String doHttpsPost = PostCLientUtils.doHttpsPost(Information.KONGCV_INSERT_ACCEPT,
							JsonStrUtils.JsonStr(object));
					Log.v("插入通知接受数据", doHttpsPost);
				} catch (Exception e) {
					e.printStackTrace();
				}
			}
